...
app.get("/", (req, res) => {
	const language =
		req.query.language || req.headers["accept-language"].split(",")[0];
		...
		const updatedData = data.map((country) => {
			...
			if (translation) {
				return {
					...country,
					name: {
						common: translation.common || country.name.common,
						official: translation.official || country.name.official,
					},
				};
			} else {
				return country;
	...
	try {
		...
		return {
			...
			possibly_extinct_in_the_wild:
				dataAnimal.possibly_extinct_in_the_wild,
		};
	} catch (error) {
...
app.get("/animals/:pays/:limiteAPI", async (req, res) => {
	...
	try {
		...
		const animalPromises = assessmentIds
			.slice(0, limiteAPI)
			.map((idAnimal) => findAnimalWithId(idAnimal));
...
app.get("/version", async (req, res) => {
	const versionInfo = {
		...
		commit_author: process.env.COMMIT_AUTHOR || "unknown",
		commit_date: process.env.COMMIT_DATE || "unknown",
	};
